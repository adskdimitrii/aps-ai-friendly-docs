...
ROOT_URL = "https://aps.autodesk.com/en/docs/viewer/v7/developers_guide/"
CUSTOM_JS_URL = "https://aps.autodesk.com/params/custom.js"
CONFIG_FILENAME = "viewer_v7.json"
OUTPUT_DIR = Path(__file__).resolve().parent / "developers-guide-docs"
TIMEOUT = 30
...
def collect_guide_leaf_nodes(config: dict) -> list[dict]:
    queue = deque([(config, "")])
    leaves: list[dict] = []
    ...
    while queue:
        ...
        if full_path.startswith("viewer/developers_guide/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def output_slug_from_full_path(full_path: str) -> str:
    prefix = "viewer/developers_guide/"
    relative_path = full_path[len(prefix) :] if full_path.startswith(prefix) else full_path
    return slug_from_path(relative_path)


def viewer_v7_doc_url(full_path: str) -> str:
    suffix = full_path.split("/", 1)[1] if full_path.startswith("viewer/") else full_path
    return f"https://aps.autodesk.com/en/docs/viewer/v7/{suffix}/"
...
def main() -> None:
    ...
    leaf_nodes = collect_guide_leaf_nodes(config.json())

    url_list_file = OUTPUT_DIR / "_url_index.md"
    index_lines = [
        "# Viewer v7 Developer's Guide URLs",
        "",
        f"Root page: {ROOT_URL}",
        f"Config source: {config_url}",
        "",
    ]
    index_lines.extend(f"- {viewer_v7_doc_url(node['full_path'])}" for node in leaf_nodes)
    ...
    for node in leaf_nodes:
        doc_url = viewer_v7_doc_url(node["full_path"])
    ...
    print(
        f"Completed. Developer guide pages found: {len(leaf_nodes)}, files written: {written}, skipped: {skipped}"
    )

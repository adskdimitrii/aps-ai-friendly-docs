...
To verify whether a user can create an RFI, call [GET users/me](/en/docs/acc/v1/reference/http/rfis-users-me-GET). If the `createRfi` object appears in the `permittedActions` response, it indicates that the user can create RFIs.

We strongly recommend calling [GET users/me](/en/docs/acc/v1/reference/http/rfis-users-me-GET) before creating RFIs in order to verify the most up-to-date permissions and actions.

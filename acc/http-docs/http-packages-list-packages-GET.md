...
| filter[versionType]   enum:string | Filters results by the version type of the packages. <br>Possible values:<br>`FIXED` â Files in the package remain fixed at selected versions.`CURRENT` â Files in the package automatically update to the latest current versions.<br>For more details, see the [Flexible Package Types](https://help.autodesk.com/view/BUILD/ENU/?guid=File_Packages_Docs) documentation. |

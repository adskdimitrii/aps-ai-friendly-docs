...
| Method and URI | GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses/:expenseId/items/:id |
| --- | --- |
...
| include   array: string | A list of the nested expense resources to include in the response with the expense items. For example, `include=budget` returns the IDs of budgets related to the expense items, `include=attributes` will return custom attributes which represents the “properties” in the response. <br>Possible values: `budget`, `contract`, `attributes`, `externalRelationship`. |
| --- | --- |

...
| Method and URI | PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/schedule-of-values/:id |
| --- | --- |
...
| --- | --- |
| id   string | The ID of the SOV item to update. |
...
| lockedField   string,null | The field of the SOV item to lock. You can lock the itemâs `quantity`, `unitPrice`, or `amount` when calculating its cost. |

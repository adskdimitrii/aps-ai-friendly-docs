...
| Method and URI | PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/change-orders/:changeOrder/:id |
| --- | --- |
...
| --- | --- |
| description   string | The description of the change order. <br>Max length: 2048 |
...
| ownerId   string,null | The ID of the change order’s owner/purchaser, a project user managed by BIM 360 Admin. |
| scopeOfWork   string | The scope of work of the change order. This is a Tiptap formatted rich text (<https://tiptap.dev/introduction/>). |
| note   string | Additional notes to the change order. This is a Tiptap formatted rich text (<https://tiptap.dev/introduction/>). |
| exchangeRate   number,string,null | A new currency exchange rate for the change order.Note that: <br>If the `changeOrder` parameter value is `rfq`, this value updates the proposed exchange rate for all cost items under this RFQ.If the `changeOrder` parameter value is `sco`, this value updates the committed exchange rate for all cost items under this SCO. |
| companyId   string,null | The BIM360/Forma ID of the company. For RFQ & SCO it’s the supplier company. For OCO and OCO it’s the owner company. For PCO, when PCO acts as RFQ it’s the supplier company, when PCO asts as RCO it’s the owner company, otherwise it’s empty. |
...
| estimated   number,string,null | Not relevant |
| proposed   number,string,null | Not relevant |
| submitted   number,string,null | Not relevant |
| approved   number,string,null | Not relevant |
| committed   number,string,null | Not relevant |

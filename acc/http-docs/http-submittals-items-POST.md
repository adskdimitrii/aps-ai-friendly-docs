...
| --- | --- |
| typeId*   string | The UUID of the item type to assign to the submittal item, which can be obtained by calling [GET item-types](http-submittals-item-types-GET.md) or [GET item-types/:id](http-submittals-item-types-id-GET.md). |
...
| subcontractor   string | The Autodesk ID of the subcontractor to assign to the submittal item. When creating a submittal item as a manager in the UI, you will see âassign to responsible contractorâ. <br>Get the subcontractor ID using:  > [GET projects/users](http-admin-projectsprojectId-users-GET.md) to verify the actual name of the user in case the typs is a `user` (1).[GET companies](http-projects--project_id-companies-GET.md) to determine the name of the company in case the typs is a `company` (2). <br>Note that we do not currently support verifying names of roles. |
...
| packageId   string: UUID | The ID of the package associated with the submittal item. <br>To retrieve a list of packages and their corresponding IDs, call [GET packages](http-submittals-packages-GET.md). |
| templateId   string: UUID | The ID of a review template used to apply predefined steps and tasks when creating a submittal item. Templates must be created in the UI or retrieved by calling [GET templates](http-submittals-templates-GET.md). |
...
| manager   string | The ID that was assigned to the manager of the submittal item. If the creator of the submittal item held a manager role, they were automatically set as the manager (`manager`). <br>To determine the type of the manager (i.e., user, role, or company), refer to the manager type (`managerType`) attribute. In order to get more info about the manager, use:  > [GET projects/users](http-admin-projectsprojectId-users-GET.md) to verify the actual name of the user in case the typs is a `user` (1).[GET companies](http-projects--project_id-companies-GET.md) to determine the name of the company in case the typs is a `company` (2). <br>Note that we do not currently support verifying names of roles. |

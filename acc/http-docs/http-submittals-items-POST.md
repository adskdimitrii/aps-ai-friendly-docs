...
For information about submittal items, see the [Help documentation](https://help.autodesk.com/view/BUILD/ENU/?guid=Create_Submittal).
...
| Method and URI | POST https://developer.api.autodesk.com/construction/submittals/v2/projects/:projectId/items |
| --- | --- |
...
| --- | --- |
| typeId*   string | The UUID of the item type to assign to the submittal item, which can be obtained by calling [GET item-types](/en/docs/acc/v1/reference/http/submittals-item-types-GET/) or [GET item-types/:id](/en/docs/acc/v1/reference/http/submittals-item-types-id-GET/). |
| specId*   string: UUID | The UUID of the spec section to assign to the submittal item. |
| subsection   string | Define the subsection details of the submittal item. For example, `1.05-B`. |
| title*   string | The title of the submittal item defined by the user when creating the submittal item. |
| description   string | A description of the submittal item to add context about its purpose. |
| priority   enum:string | The priority level of the submittal item. Possible values: `Low`, `Normal`, `High`. <br>When the priority is not set, the default value is `Normal`. |
| stateId*   enum:string | The initial state of the submittal item, which depends on the role and permitted actions of the user. <br>Possible values: `draft`, `sbc-1`, `mgr-1`.<br>The required fields depend on the selected state ID. For example, if the state is `mgr-1`, required fields include subcontractor (`subcontractor`), subcontractor type (`subcontractorType`), and submitter due date (`submitterDueDate`). If the state is `sbc-1`, required fields include manager (`manager`) and manager type (`manager-type`).<br>The required fields for creating the submittal item depend on the input stateId. The different states will require different mandatory fields. Refer Step 1: Verify User Permissions in the [Create a Submittal Item](/en/docs/acc/v1/tutorials/submittals/create-submittal-item/) tutorial for details. |
| manager   string | The Autodesk ID (`autodeskId`) or Group ID (`memberGroupId`) specifying the manager to assign to the submittal item. The manager could be a user, role, or a company. Note that the user must be defined as a manager in the project. You can verify this by calling [GET mappings](/en/docs/acc/v1/reference/http/submittals-mappings-GET/). <br>To get the possible values for a manager, call [GET mappings](/en/docs/acc/v1/reference/http/submittals-mappings-GET/) and check the autodeskId. This returns a list of all the users, roles, and companies that are managers in the project. Call [GET projects/users](/en/docs/acc/v1/reference/http/admin-projectsprojectId-users-GET) to verify the actual name of the user. If a company is a manager, call [GET companies](en/docs/acc/v1/reference/http/projects-:project_id-companies-GET/) to verify the actual name of the company. Note that we do not currently support verifying names of roles. |
...
| subcontractor   string | The Autodesk ID of the subcontractor to assign to the submittal item. When creating a submittal item as a manager in the UI, you will see âassign to responsible contractorâ. <br>Get the subcontractor ID using:  > [GET projects/users](/en/docs/acc/v1/reference/http/admin-projectsprojectId-users-GET) to verify the actual name of the user in case the typs is a `user` (1).[GET companies](/en/docs/acc/v1/reference/http/projects-:project_id-companies-GET/) to determine the name of the company in case the typs is a `company` (2). <br>Note that we do not currently support verifying names of roles. |
| subcontractorType   enum:string | The type of subcontractor. This attribute is required when a subcontractor is specified. Possible values: `1` (user), `2` (company), `3` (role). |
...
| managerDueDate   string | The date by which the manager is expected to prepare the submittal item for review to the reviewer, in the following format: YYYY-MM-DD in UTC (ISO 8601). For example, `2018-02-15`. This corresponds to the `mgr-1` state `Open (Submitted)`. |
| packageId   string: UUID | The ID of the package associated with the submittal item. <br>To retrieve a list of packages and their corresponding IDs, call [GET packages](/en/docs/acc/v1/reference/http/submittals-packages-GET/). |
| templateId   string: UUID | The ID of a review template used to apply predefined steps and tasks when creating a submittal item. Templates must be created in the UI or retrieved by calling [GET templates](/en/docs/acc/v1/reference/http/submittals-templates-GET/). |
...
| manager   string | The ID that was assigned to the manager of the submittal item. If the creator of the submittal item held a manager role, they were automatically set as the manager (`manager`). <br>To determine the type of the manager (i.e., user, role, or company), refer to the manager type (`managerType`) attribute. In order to get more info about the manager, use:  > [GET projects/users](/en/docs/acc/v1/reference/http/admin-projectsprojectId-users-GET) to verify the actual name of the user in case the typs is a `user` (1).[GET companies](/en/docs/acc/v1/reference/http/projects-:project_id-companies-GET/) to determine the name of the company in case the typs is a `company` (2). <br>Note that we do not currently support verifying names of roles. |

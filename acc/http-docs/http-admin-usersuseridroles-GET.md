...
Only users with account admin permissions can call this endpoint. To verify a userâs permissions, call [GET users](/en/docs/acc/v1/reference/http/admin-projectsprojectId-users-GET/).
...
| --- | --- |
| filter[status]   array: string | Filters roles by their status. Accepts one or more of the following values: <br>`active` â The role is currently in use.<br>`inactive` â The role has been removed or is no longer in use. |
...
| fields   array: string | A comma-separated list of response fields to include. Defaults to all fields if not specified. <br>Use this parameter to reduce the response size by retrieving only the fields you need.<br>Possible values:<br>`projectIds` â Projects where the user holds this role<br>`name` â Role name<br>`status` â Role status (active or inactive)<br>`key` â Internal key used to translate the role name<br>`createdAt` â Timestamp when the role was created<br>`updatedAt` â Timestamp when the role was last updated |
| sort   array: string | Sorts the results by one or more fields. <br>Each field can be followed by a direction modifier:<br>`asc` â Ascending order (default)<br>`desc` â Descending order<br>Possible values: `name`, `createdAt`, `updatedAt`.<br>Default sort: `name asc`<br>Example: `sort=name,updatedAt desc` |

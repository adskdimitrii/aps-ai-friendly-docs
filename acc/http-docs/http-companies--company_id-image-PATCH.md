...
| Method and URI | PATCH https://developer.api.autodesk.com/hq/v1/accounts/:account_id/companies/:company_id/image |
| --- | --- |
| Method and URI (Legacy) | PATCH https://developer.api.autodesk.com/hq/v1/regions/eu/accounts/:account_id/companies/:company_id/image |
...
| --- | --- | --- | --- |
| Authorization | string | yes | Must be `Bearer <token>`, where `<token>` is obtained via a [two-legged](/en/docs/oauth/v2/tutorials/get-2-legged-token) OAuth flow. |
| Region | string | no | Specifies the region where the service is located. Possible values: `US`, `EMEA`. For the full list of supported regions, see the [Regions](/en/docs/acc/v1/overview/acc-regions) page. |

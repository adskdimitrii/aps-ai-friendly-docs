...
For more details about creating and managing cost items, see the Cost Management Help documentation .

## Resource Information

Method and URI POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items:batch-create Authentication Context user context required Required OAuth Scopes data:write Data Format JSON
...
data * array: object A list of cost items to be created in the project. name * string The name of the cost item to be created. Max length: 1024 changeOrderId string,null The ID of the change order in which the cost item is created. Note that once a cost item is associated with a change order, it cannot be reassigned to a different change order. Call GET change-orders to retrieve the change order ID. budgetId string,null The ID of the budget to which the cost item will be linked. Call GET budgets to retrieve the budget ID. contractId string,null The ID of the contract to which the cost item will be linked. Call GET contracts to retrieve the contract ID. description string A detailed description of the cost item. Max length: 2048 estimated number,string,null The estimated value of the cost item provided by the general contractor ( GC ). proposed number,string,null The quoted value of the cost item provided by the supplier in response to a Request For Quotation ( RFQ ). submitted number,string,null The value submitted to the owner for review as part of a Change Order Request ( COR ). approved number,string,null The value approved by the owner and recorded in the Owner Change Order ( OCO ). committed number,string,null The value committed to the supplier after the Subcontractor Change Order ( SCO ) is signed. inputQuantity number,null The initial quantity of the cost item as specified in the budget or contract. quantity number The total quantity of the cost item to be used. unit string The unit of measurement for the cost item. proposedExchangeRate number,string,null The exchange rate for the proposed cost item. If multi-currency is not enabled, the value is set to 1 regardless of the specified rate. Default value: 1 . For more details, see the Help documentation . committedExchangeRate number,string,null The exchange rate for the committed cost item. If multi-currency is not enabled, the value is set to 1 regardless of the specified rate. Default value: 1 . For more details, see the Help documentation . locations array,null A list of the IDs of the project locations where this item applies. For more information, see the Locations Help documentation help. externalId string The identifier assigned to an item in its original external ERP system. Use this ID to track and look up data within the integrated system. Note that this value comes from the itemâs ID in the external system. Max length: 255 externalSystem string The name of the external ERP system integrated with Cost Management. Use this name to identify and search for data within the integrated system. Max length: 255 externalMessage string A message generated by the external ERP system that explains the sync status of the integration. For example, common values include success or fail to indicate the result of the integration operation. Max length: 255 lastSyncTime datetime: ISO 8601 The date and time when the item was last synchronized with the external ERP system. This value is updated by the external system and is in ISO 8601 format. integrationState string,null The state of the item during the integration with the external ERP system (such as SignNow). An item can be a budget , contract , main contract , main contract item , cost item , expense , expense item , change order , or schedule of value . For more details, see Integrate with External System tutorial.
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/cost-items:batch-create' \ -X 'POST' \ -H 'Authorization: Bearer AuIPTf4KYLTYGVnOHQ0cuolwCW2a' \ -H 'Content-Type: application/json' \ -d '{ "data": [ { "name": "Revised Entry Details per RFI-001", "changeOrderId": "f0c3c060-2a8d-11ee-b1a7-2f4a196105cb", "budgetId": "227e1360-9481-11e8-87fb-215990a8aeb3", "contractId": "48dba1a0-2a8e-11ee-b1a7-2f4a196105cb", "description": "Revised Entry Details per RFI-001", "estimated": "1000.0000", "proposed": "1000.0000", "submitted": "1000.0000", "approved": "1000.0000", "committed": "1000.0000", "inputQuantity": 1, "quantity": 1, "unit": "ls", "proposedExchangeRate": "1.0000", "committedExchangeRate": "1.0000", "locations": [ "683904a0-47ce-4146-ac2d-a3840f00e0f4" ], "externalId": "10010-99-AB", "externalSystem": "Sage300", "externalMessage": "Success.", "lastSyncTime": "2019-09-05T01:00:12.989Z", "integrationState": "locked" } ] }'
```

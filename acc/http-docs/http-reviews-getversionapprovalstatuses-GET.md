...
- versionIdstring The URL-encoded unique identifier (URN) of the file version whose review and approval history you want to retrieve. For example, encode `urn:adsk.wipprod:fs.file:vf.Ibsc4cPuQEqBHRJdBjhr6w?version=2``as ``urn%3Aadsk.wipprod%3Afs.file%3Avf.Ibsc4cPuQEqBHRJdBjhr6w%3Fversion%3D2`.To find the latest version, call [GET versions](/en/docs/acc/v1/reference/http/reviews-getreviewversions-GET/) and check the `urn` field.

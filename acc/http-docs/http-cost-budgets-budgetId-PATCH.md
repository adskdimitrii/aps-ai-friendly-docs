...
| actualQuantity   number | Actual quantity of labor, material,… planned for a budget. |
...
| lockedField   string,null | The locked budget item field. You can lock the budget item’s amount (`originalAmount`), quantity (`quantity`), or unit cost (`unitPrice`) when calculating a budget. |

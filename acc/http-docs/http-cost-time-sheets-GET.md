...
| filter[budgetCode]   string | Filter data that belong to associated budget code. For example, `filter[budgetCode]=code`. <br>To find budget codes, call [GET budgets](http-cost-budgets-GET.md) and inspect `results.code` in the response. |
...
| creatorId   string,null | The BIM 360/Forma ID of the user who created the timesheet. |
| changedBy   string,null | The BIM 360/Forma ID of the user who made the last change to the timesheet. |

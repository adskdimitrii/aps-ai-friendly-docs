...
To check if a project contains takeoff types and items, call GET packages and use the package IDs ( results[i].id ) to call GET takeoff-types .

## Resource Information

Method and URI PATCH https://developer.api.autodesk.com/construction/takeoff/v1/projects/{projectId}/settings Authentication Context user context required Required OAuth Scopes data:write Data Format JSON
...
```
curl -v 'https://developer.api.autodesk.com/construction/takeoff/v1/projects/:projectId/settings' \ -X 'PATCH' \ -H 'Authorization: Bearer AuIPTf4KYLTYGVnOHQ0cuolwCW2a' \ -H 'Content-Type: application/json' \ -d '{ "measurementSystem": "IMPERIAL" }'
```

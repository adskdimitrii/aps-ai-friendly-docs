...
To retrieve the exact workflow that was applied to a specific review, call [GET reviews/:reviewId/workflow](http-reviews-getreviewworkflow-GET.md).

Note that this endpoint is not compatible with BIM 360 projects.

## [Resource Information](#resource-information)

| Method and URI | GET https://developer.api.autodesk.com/construction/reviews/v1/projects/{projectId}/workflows |
| --- | --- |
...
| filter[initiator]   boolean | Filters the results based on who initiated the workflow. For example: `filter[initiator]=true`. <br>`true`: return only workflows initiated by the current user. This filter is ignored if the user is a project admin.<br>`false`: (default) return workflows regardless of who initiated them.<br>Note that this filter cannot be used together with `filter[status]`. |
| filter[status]   enum:string | Filters the results by workflow status. For example: `filter[status]=INACTIVE`. Possible values: <br>`ACTIVE`: return only active workflows.<br>`INACTIVE`: return only inactive (disabled) workflows.<br>Default: `ACTIVE`.<br>Note that this filter cannot be used together with `filter[initiator]`. |
...
| results   array: object | A list of approval workflows in the project. |
| --- | --- |
...
| nextUrl   string | The URL to retrieve the next page of workflows results, if any. If not included, this is the last page. |

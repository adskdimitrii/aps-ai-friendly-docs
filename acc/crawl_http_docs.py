...
ROOT_URL = "https://aps.autodesk.com/en/docs/acc/v1/reference/http/"
...
def collect_http_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("acc/reference/http/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def main() -> None:
    ...
    index_lines = ["# ACC HTTP API Doc URLs", "", f"Config source: {config_url}", ""]
    index_lines.extend(

...
- You need to use the project ID instead of the container ID. To obtain the project ID, use [GET projects](/en/docs/bim360/v1/reference/http/admin-accounts-accountidprojects-GET/).
...
- Acquire a [3-legged OAuth token](/en/docs/oauth/v2/tutorials/get-3-legged-token/) or a [2-legged OAuth token with user impersonation](/en/docs/oauth/v2/tutorials/get-2-legged-token/) with the `data:read` scope.
...
You must first call [GET hubs](/en/docs/data/v2/reference/http/hubs-GET/) to find the account ID for your BIM 360 account.
...
Use the BIM 360 hub ID that you retrieved in the previous step (`b.cGVyc29uYWw6cGUyOWNjZjMy`) to call [GET hubs/:hub_id/projects](/en/docs/data/v2/reference/http/hubs-hub_id-projects-GET), and retrieve a list of all the projects in the hub to which the user has access. In this example, we have added a filter to return only details of a specific project âDemo Project.â

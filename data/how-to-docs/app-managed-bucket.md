...
Make sure that you have [registered an app](/myapps) and successfully [acquired an OAuth token](/en/docs/oauth/v2/tutorials/get-2-legged-token) with scopes `bucket:create`, `bucket:read`, and `data:write`. This will allow you to create a bucket, get bucket details, and upload a file.

## [Step 1: Create a bucket](#step-1-create-a-bucket)

Before uploading a file, create a bucket and set a retention policy using the [POST buckets](/en/docs/data/v2/reference/http/buckets-POST) endpoint. In our example, weâre going to create a `transient` bucket, meaning that anything uploaded to it will be deleted after 24 hours.
...
Once the bucket is created, verify that it exists using the [GET buckets/:bucketKey/details](/en/docs/data/v2/reference/http/buckets-:bucketKey-details-GET) endpoint:
...
## [Step 2: Initiate a direct to s3 multipart upload](#step-2-initiate-a-direct-to-s3-multipart-upload)
...
The [GET buckets/:bucketKey/objects/:objectKey/signeds3upload](/en/docs/data/v2/reference/http/buckets-:bucketKey-objects-:objectKey-signeds3upload-GET) requests one or multiple S3 signed URL with which to upload an object based on how many âpartsâ you specify. These URLs will directly transmit the file to S3 and will be registered in OSS once [POST buckets/:bucketKey/objects/:objectKey/signeds3upload](/en/docs/data/v2/reference/http/buckets-:bucketKey-objects-:objectKey-signeds3upload-POST) is called.
...
## [Step 3: Split the file, and upload.](#step-3-split-the-file-and-upload)
...
Call the [POST buckets/:bucketKey/objects/:objectKey/signeds3upload](/en/docs/data/v2/reference/http/buckets-:bucketKey-objects-:objectKey-signeds3upload-POST) endpoint to complete the multipart upload and register the item with OSS.

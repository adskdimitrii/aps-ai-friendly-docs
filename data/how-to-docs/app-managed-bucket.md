...
Before uploading a file, create a bucket and set a retention policy using the [POST buckets](../http-docs/http-buckets-POST.md) endpoint. In our example, we’re going to create a `transient` bucket, meaning that anything uploaded to it will be deleted after 24 hours.

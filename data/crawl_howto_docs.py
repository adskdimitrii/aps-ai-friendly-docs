...
def collect_howto_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("data/tutorials/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def data_v2_doc_url(full_path: str) -> str:
    suffix = full_path.split("/", 1)[1] if full_path.startswith("data/") else full_path
    return f"https://aps.autodesk.com/en/docs/data/v2/{suffix}/"
...
def main() -> None:
    ...
    index_lines = [
        "# Data How-to Guide URLs",
        "",
        f"Root page: {ROOT_URL}",
        f"Config source: {config_url}",
        "",
    ]
    index_lines.extend(f"- {data_v2_doc_url(node['full_path'])}" for node in leaf_nodes)
    ...
    for node in leaf_nodes:
        doc_url = data_v2_doc_url(node["full_path"])

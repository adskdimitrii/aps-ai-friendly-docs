...
ROOT_URL = "https://aps.autodesk.com/en/docs/tokenflex/v1/tutorials/"
...
def collect_tutorial_leaf_nodes(config: dict) -> list[dict]:
    queue = deque([(config, "")])
    leaves: list[dict] = []
    ...
    while queue:
        ...
        if full_path.startswith("tokenflex/tutorials/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def main() -> None:
    ...
    leaf_nodes = collect_tutorial_leaf_nodes(config.json())

    url_list_file = OUTPUT_DIR / "_url_index.md"
    index_lines = [
        "# Token Flex Tutorials URLs",
        "",
    ...
    print(f"Completed. Tutorial pages found: {len(leaf_nodes)}, files written: {written}, skipped: {skipped}")

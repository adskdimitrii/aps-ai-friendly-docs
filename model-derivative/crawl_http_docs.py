...
ROOT_URL = "https://aps.autodesk.com/en/docs/model-derivative/v2/reference/http/"
...
def collect_http_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("model-derivative/reference/http/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def main() -> None:
    ...
    index_lines = [
        "# Model Derivative HTTP API Doc URLs",
        "",

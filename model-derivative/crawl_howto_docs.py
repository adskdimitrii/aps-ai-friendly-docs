...
ROOT_URL = "https://aps.autodesk.com/en/docs/model-derivative/v2/tutorials/"
CUSTOM_JS_URL = "https://aps.autodesk.com/params/custom.js"
CONFIG_FILENAME = "model-derivative_v2.json"
OUTPUT_DIR = Path(__file__).resolve().parent / "how-to-docs"
TIMEOUT = 30
...
def collect_howto_leaf_nodes(config: dict) -> list[dict]:
    queue = deque([(config, "")])
    leaves: list[dict] = []
    ...
    while queue:
        ...
        if full_path.startswith("model-derivative/tutorials/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def model_derivative_v2_doc_url(full_path: str) -> str:
    suffix = full_path.split("/", 1)[1] if full_path.startswith("model-derivative/") else full_path
    return f"https://aps.autodesk.com/en/docs/model-derivative/v2/{suffix}/"
...
def main() -> None:
    ...
    leaf_nodes = collect_howto_leaf_nodes(config.json())

    url_list_file = OUTPUT_DIR / "_url_index.md"
    index_lines = [
        "# Model Derivative How-to Guide URLs",
        "",
        f"Root page: {ROOT_URL}",
        f"Config source: {config_url}",
        "",
    ]
    index_lines.extend(f"- {model_derivative_v2_doc_url(node['full_path'])}" for node in leaf_nodes)
    ...
    for node in leaf_nodes:
        doc_url = model_derivative_v2_doc_url(node["full_path"])
    ...
    print(f"Completed. How-to pages found: {len(leaf_nodes)}, files written: {written}, skipped: {skipped}")

...
ROOT_URL = "https://aps.autodesk.com/en/docs/design-automation/v3/tutorials/"
CUSTOM_JS_URL = "https://aps.autodesk.com/params/custom.js"
CONFIG_FILENAME = "design-automation_v3.json"
...
def collect_howto_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("design-automation/tutorials/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def design_automation_v3_doc_url(full_path: str) -> str:
    suffix = full_path.split("/", 1)[1] if full_path.startswith("design-automation/") else full_path
    return f"https://aps.autodesk.com/en/docs/design-automation/v3/{suffix}/"
...
def main() -> None:
    ...
    index_lines = [
        "# Design Automation How-to Guide URLs",
        "",
    ...
    index_lines.extend(
        f"- {design_automation_v3_doc_url(node['full_path'])}" for node in leaf_nodes
    )
    ...
    for node in leaf_nodes:
        doc_url = design_automation_v3_doc_url(node["full_path"])
    ...
    print(
        f"Completed. How-to pages found: {len(leaf_nodes)}, files written: {written}, skipped: {skipped}"
    )

...
Method and URI POST https://developer.api.autodesk.com/da/us-east/v3/activities/:id/versions Authentication Context app only Required OAuth Scopes code:all Data Format JSON
...
200 OK Successfully create a new verison of an Activity. 400 Bad Request The request is invalid. 403 Forbidden Limit for number of versions exceeded. 404 Not Found Could not find the item. 409 Conflict The item version already exist. (this might happen if 2 requests are done in parallel to create the same version). 500 Internal Server Error Internal error.

...
| Method and URI | GET https://developer.api.autodesk.com/webhooks/v1/app/hooks |
| --- | --- |
| Authentication Context | app only (2-legged token) |
...
| Authorization*   string | Must be `Bearer <token>`, where `<token>` is obtained via [OAuth](/en/docs/oauth/v2/reference/http/gettoken-POST). A 2-legged token is required for this API. |
| --- | --- |

...
```
package main import ( "fmt" "io/ioutil" "log" "net/http" ) const PORT = 5678 func callbackHandler ( w http . ResponseWriter , req * http . Request ) { body , _ := ioutil . ReadAll ( req . Body ) fmt . Printf ( "Payload \n %s" , body ) } func main () { http . HandleFunc ( "/callback" , callbackHandler ) log . Fatal ( http . ListenAndServe ( fmt . Sprintf ( ":%v" , PORT ), nil )) }
```

Alternatively, you can learn more about Sinatra and find a Sinatra application here .
...
You can use http://bf067e05.ngrok.io/callback in Step 4 for creating a hook.

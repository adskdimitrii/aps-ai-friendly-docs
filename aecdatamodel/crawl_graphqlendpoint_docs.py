...
ROOT_URL = "https://aps.autodesk.com/en/docs/aecdatamodel/v1/reference/graphqlendpoint/"
...
def collect_graphqlendpoint_nodes(config: dict) -> list[dict]:
    queue = deque([(config, "")])
    leaves: list[dict] = []
    ...
    while queue:
        ...
        if full_path == "aecdatamodel/reference/graphqlendpoint" and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def main() -> None:
    ...
    leaf_nodes = collect_graphqlendpoint_nodes(config.json())

    url_list_file = OUTPUT_DIR / "_url_index.md"
    index_lines = [
        "# AEC Data Model GraphQL Endpoint Reference URLs",
        "",
    ...
    print(f"Completed. GraphQL Endpoint pages found: {len(leaf_nodes)}, files written: {written}, skipped: {skipped}")

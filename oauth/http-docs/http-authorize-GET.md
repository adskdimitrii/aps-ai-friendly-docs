...
| prompt   string | The application prompts the user in a certain way based on its value. Valid value: `login` and `create`       If no `prompt` parameter is specified, the standard behavior occurs as follows:   If an user session already exists, the user is authenticated else, the user is prompted to authenticate.       `login`: Always prompt the user for authentication, regardless of the login session.   `create`: Request to display user registration/account creation screen instead of the standard login screen. |

...
ROOT_URL = "https://aps.autodesk.com/en/docs/oauth/v2/developers_guide/overview/"
CUSTOM_JS_URL = "https://aps.autodesk.com/params/custom.js"
CONFIG_FILENAME = "oauth_v2.json"
...
def collect_guide_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("oauth/developers_guide/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def oauth_v2_doc_url(full_path: str) -> str:
    suffix = full_path.split("/", 1)[1] if full_path.startswith("oauth/") else full_path
    return f"https://aps.autodesk.com/en/docs/oauth/v2/{suffix}/"
...
def main() -> None:
    ...
    index_lines = [
        "# OAuth Developer's Guide URLs",
        "",
        f"Root page: {ROOT_URL}",
        f"Config source: {config_url}",
        "",
    ]
    index_lines.extend(f"- {oauth_v2_doc_url(node['full_path'])}" for node in leaf_nodes)
    ...
    for node in leaf_nodes:
        doc_url = oauth_v2_doc_url(node["full_path"])

...
ROOT_URL = "https://aps.autodesk.com/en/docs/oauth/v2/reference/http/"
...
def collect_http_leaf_nodes(config: dict) -> list[dict]:
    ...
    while queue:
        ...
        if full_path.startswith("oauth/reference/http/") and source:
            leaves.append(
                {
                    "full_path": full_path,
                    "url_path": url_path,
...
def main() -> None:
    ...
    index_lines = ["# OAuth HTTP API Doc URLs", "", f"Root page: {ROOT_URL}", f"Config source: {config_url}", ""]
